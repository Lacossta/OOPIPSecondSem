...
// --- Containers ---
map<string, MobileApp> appMap;                                     // уникальные приложения
set<MobileApp> appSet;                                             // уникальные сортированные
unordered_map<string, MobileApp> appUnorderedMap;                  // быстрый досутп по имени
multimap<string, MobileApp> appMultimap;                           // поддержка дублирования по ключу
unordered_multiset<MobileApp, MobileAppHash> appUnorderedMultiset; // дублирование и быстрая вставка

// add
void addApp(map<string, MobileApp>& container, const MobileApp& app) {
    container[app.name] = app; // В map добавляем или обновляем объект
    appSet.insert(app); // В set добавляем уникальный объект
    appUnorderedMap[app.name] = app; // Быстрый доступ
    appMultimap.insert({app.name, app}); // Возможность хранить дубликаты
    appUnorderedMultiset.insert(app); // Дублирование без строгого порядка
}
...
void sortAppsByName(map<string, MobileApp>& container) {
    // map уже хранит данные в отсортированном порядке, дополнительных действий не нужно
    ...
    while (true) {
        ...
        switch (choice) {
            case 1: {
                ...
                addApp(appMap, MobileApp(name, version, releaseDate, status, developer, platform));
                break;
            }
            case 2: {
                string name;
